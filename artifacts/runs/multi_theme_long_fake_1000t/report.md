# Multi-Theme Long LLM Benchmark

Verdict: VALIDATING
Model: `contract_aware_fake`

| Turns | Answer OK | Source attr | Hallucination | Coherence | Long-range | Avoided input | Prompt distinct | Context p95 |
|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 1000 | 100.00% | 100.00% | 0.00% | 100.00% | 100.00% | 97.45% | 100.00% | 2.27 ms |

Average baseline blocks: `252.00`.
Average Highway blocks: `2.00`.
Poison fail rate: `0.00%`.

Metrics JSON: `artifacts/runs/multi_theme_long_fake_1000t/metrics.json`
Records JSONL: `artifacts/runs/multi_theme_long_fake_1000t/records.jsonl`
