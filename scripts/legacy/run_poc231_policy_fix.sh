...
#############################################
# POC 2.3.1 - SAFE EXECUTION POLICY FIX
# 200 queries / 3 modes / timeout 45m
# Focus: G/H correctness + A-F coverage
#############################################

RUN_NAME="poc_2_3_1_policy_fix"
RUN_DIR="artifacts/runs/${RUN_NAME}"
WORKLOAD_DIR="data/workloads"
WORKLOAD_FILE="${WORKLOAD_DIR}/mixed_runtime_200_policy_fix.jsonl"
...
echo "==================================================" | tee "${LOG_FILE}"
echo "POC 2.3.1 Safe Execution Policy Fix Benchmark" | tee -a "${LOG_FILE}"
...
if [ ! -f "build_poc231_workload.py" ]; then
  echo "ERROR: Missing build_poc231_workload.py" | tee -a "${LOG_FILE}"
...
echo "Clearing old results and cache..." | tee -a "${LOG_FILE}"
rm -f "${RESULTS_FILE}"
...
echo "" | tee -a "${LOG_FILE}"
echo "[1/6] Building 200-query focused workload..." | tee -a "${LOG_FILE}"

"${PYTHON}" -u build_poc231_workload.py \
  --corpus "${CORPUS_DIR}" \
...
{
  echo "# POC 2.3.1 Policy Fix â€” Quick Summary"
  echo ""
...
echo "" | tee -a "${LOG_FILE}"
echo "==================================================" | tee -a "${LOG_FILE}"
echo "POC 2.3.1 Run finished at: $(date)" | tee -a "${LOG_FILE}"
echo "==================================================" | tee -a "${LOG_FILE}"
