...
def main():
    parser = argparse.ArgumentParser(description="POC 1.3 Adaptive Smoke Test (All Categories)")
    parser.add_argument("--run-name", type=str, default="poc_1_3_smoke_all")
    ...
    parser.add_argument("--modes", type=str, default="predictor_cached_guarded_json,predictor_cached_guarded_adaptive_kernel")
    parser.add_argument("--temperature", type=float, default=0.0)
    parser.add_argument("--top-p", type=float, default=1.0)
    parser.add_argument("--max-new-tokens", type=int, default=64)
    parser.add_argument("--block-size", type=int, default=128)
    parser.add_argument("--output", type=str, default="artifacts/runs/poc_1_3_smoke_all/results.jsonl")
    ...
    print(f"=== Running POC 1.3 Adaptive Smoke Test: {args.run_name} ===")
    ...
    # Filter and select exactly 10 samples per Category A-E
    ...
    for idx, (size, mode, sample) in enumerate(all_combos):
        ...
        prompt_text = ""
        if mode == "predictor_cached_guarded_json":
            prompt_text = assemble_prompt_baseline(kept_block_ids, question, documents)
        elif mode == "predictor_cached_guarded_adaptive_kernel":
        ...
        # Apply Regex Postcheck automatically in adaptive kernel mode
        if mode == "predictor_cached_guarded_adaptive_kernel" and not is_guarded_abstain and not oom:
        ...
        if (idx+1) % 10 == 0:
            print(f"[{idx+1}/{len(all_combos)}] size={size}, mode={mode}, cat={category}, q_id={q_id} -> EM: {exact_match}, Err: {error_type}")

    kill_vllm_server()
    print("=== POC 1.3 smoke test complete ===")
