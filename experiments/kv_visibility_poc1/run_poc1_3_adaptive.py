...
# 1. Intent Analyzer
def analyze_intent(question: str, project_name: str) -> Dict[str, Any]:
    question_lower = question.lower()
    required_fields = []
    if "budget" in question_lower or "cost" in question_lower:
        required_fields.append("budget")
    if "deadline" in question_lower or "date" in question_lower or "launch" in question_lower or "deliver" in question_lower:
    ...
    return {
        ...
        "intent": "multi_fact_numeric_extraction" if len(required_fields) > 1 else "single_fact_extraction",
        "required_fields": required_fields,
        "forbidden_matches": forbidden_matches,
        "answer_format": "DATE and BUDGET" if len(required_fields) > 1 else "DATE"
    }

# 2. Structured Context Payload Compiler
def compile_structured_payload(documents: List[Dict[str, Any]], kept_block_ids: List[int], project_name: str) -> str:
...
# 3. Prompt Assemblers
def assemble_prompt_baseline(kept_ids: List[int], question: str, documents: List[Dict[str, Any]]) -> str:
    # Match the baseline assembly format from POC 1.2
    system_text = (
        "<|im_start|>system\n"
...
def assemble_prompt_kernel_only(kernel: Dict[str, Any], question: str) -> str:
    prompt = (
        "<|im_start|>system\n"
        ...
        "<|im_end|>\n"
        "<|im_start|>user\n"
        "No evidence context payload provided.\n"
...
def postprocess_regex(answer_text: str, evidence_text: str) -> str:
...
def main():
    parser = argparse.ArgumentParser(description="POC 1.3 Adaptive Context Kernel")
    parser.add_argument("--run-name", type=str, default="poc_1_3_adaptive")
    parser.add_argument("--engine", type=str, default="vllm")
    parser.add_argument("--model", type=str, default="Qwen/Qwen2.5-0.5B-Instruct")
    parser.add_argument("--contexts", type=str, default="200")
    parser.add_argument("--samples-per-combination", type=int, default=100)
    parser.add_argument("--modes", type=str, default="current_json_context,kernel_only,kernel_structured_payload,kernel_structured_payload_regex_postcheck")
    parser.add_argument("--temperature", type=float, default=0.0)
    parser.add_argument("--top-p", type=float, default=1.0)
    parser.add_argument("--max-new-tokens", type=int, default=64)
    parser.add_argument("--block-size", type=int, default=128)
    parser.add_argument("--save-every", type=int, default=10)
    parser.add_argument("--output", type=str, default="artifacts/runs/poc_1_3_adaptive/results.jsonl")
    ...
    print(f"=== Running POC 1.3 Adaptive Benchmark: {args.run_name} ===")
    ...
    # Filter for Category D
    cat_d_samples = [s for s in corpus_samples if s["category"] == "D"]
    cat_d_samples.sort(key=lambda x: x["question_id"])
    
    # Select requested count
    selected_samples = cat_d_samples[:args.samples_per_combination]
    print(f"Loaded {len(selected_samples)} Category D benchmark samples.")
    ...
    if os.path.exists(args.output):
        # Clear output file for fresh POC 1.3 run
    ...
    for size in context_sizes:
        for mode in modes:
            for sample in selected_samples:
                all_combos.append((size, mode, sample))
    ...
    for idx, (size, mode, sample) in enumerate(all_combos):
        ...
        kernel = analyze_intent(question, project_name)
        kernel["exact_entity_found"] = bool(any_exact)
        kernel["guard_status"] = "LLM_BYPASS" if is_guarded_abstain else "LLM_ALLOWED"
        
        # Compile Payload
        payload = compile_structured_payload(documents, kept_block_ids, project_name)
        
        # Assemble Prompt depending on Mode
        prompt_text = ""
        if mode == "current_json_context":
            prompt_text = assemble_prompt_baseline(kept_block_ids, question, documents)
        elif mode == "kernel_only":
            prompt_text = assemble_prompt_kernel_only(kernel, question)
        elif mode in ["kernel_structured_payload", "kernel_structured_payload_regex_postcheck"]:
        ...
        # Apply Regex Postcheck if mode requires it
        if mode == "kernel_structured_payload_regex_postcheck" and not is_guarded_abstain and not oom:
            raw_answer = parsed["answer"]
            refined_answer = postprocess_regex(raw_answer, payload)
        ...
        if (idx+1) % 10 == 0:
            print(f"[{idx+1}/{len(all_combos)}] mode={mode}, q_id={q_id} -> EM: {exact_match}, Err: {error_type}")

    kill_vllm_server()
    print("=== POC 1.3 adaptive run complete ===")
