...
def clear_embedding_cache():
    global _block_embedding_cache
    _block_embedding_cache.clear()
...
def extract_block_features(
    ...
    skip_embedding_compute: bool = False,
    cached_block_embs: np.ndarray = None
) -> np.ndarray:
    ...
    # 2. Semantic Scores
    if skip_embedding_compute and cached_block_embs is not None:
        block_embs = cached_block_embs
        # We still need query embedding but we skip block embedding compute time
        model = get_embedding_model()
